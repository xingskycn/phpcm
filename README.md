...
1.1. Размер каждой записи в memcached будет больше минимум на N байт чем без использования PHP Cache Manager.
1.2. Из за DEP-заголовка, невозможно использовать increment и decrement, реализованные внутри memcached, к данным сохраненным с помощью PHP Cache Manager.
1.3. Использовать стандартный модуль nginx для работы с memcached напрямую, т.к. в nginx помимо сохраненного html будет отдаватся DEP-заголовок
